...
def _model_cache_root() -> Path:
    ...
    try:
        ...
        # Fallback to a writable location (e.g., container /tmp) if the default is read-only.
        cache_root = Path("/tmp/disease_detection/pretrained")
        cache_root.mkdir(parents=True, exist_ok=True)
    return cache_root
...
def ensure_mobileclip_checkpoint(model_name: str = "MobileCLIP-S2") -> str:
    ...
    cache_dir = cache_root / model_name.lower()
    try:
